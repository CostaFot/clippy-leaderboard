...
- **The front page is server-rendered; the only JavaScript is the Umami tracker.** External requests are the Google Fonts stylesheet and Umami's `script.js` (Costa's self-hosted instance, `umami-production-ed35.up.railway.app`, website `graveyard`); the rest is same-origin (`/static/clippy.*`). Nothing on the page depends on JS — it renders and works identically with the tracker blocked. Keep it that way: no JS that the page itself needs.
